...
manager resumes the breakdown from there. The on-disk `.task`/`.done` stay the
source of truth for each task's *state*; the queue is the *plan*.

## Delegate

Create and delegate each task by the normal flow (see "Delegate" in
`references/roles/manager.md`): write `thoughts/<Worker>.task`, then
`delegate-task.sh <Worker>`. Route only to available workers; fit is a tiebreak,
never a fence.

## Verify against the plan

As `.done`s land, do not just check each task in isolation — compare the work
back to `thoughts/plan-<topic>.md`. Where the build reveals the design was wrong
or incomplete, iterate **both** the plan and the queue with the user, then cut
