...
*worker* and is the manager's job alone. Subagents are disposable,
...
3. **The brief is self-contained.** Each subagent gets **only what you put in
   ...
   return. A subagent must **never** read `notes-*.md` — that is the
   manager/worker contract and will only confuse it. If the prompt doesn't say
   it, the subagent doesn't know it.
